...
--    :Lazy update
---- 🧩 Helper to auto-inject logging into plugins
local function log_plugin_load(name)
  return function()
    vim.schedule(function()
      vim.notify('Hello from ' .. name, vim.log.levels.INFO, { title = 'Lazy.nvim' })
    end)
  end
end
-- Hook into all plugin specs to add greeting messages
local original_spec = {
  -- 👇 your whole plugin list goes here (copy it out of `require("lazy").setup(...)`)
}

for _, plugin in ipairs(original_spec) do
  local name = type(plugin) == 'string' and plugin or plugin[1]
  if type(plugin) == 'table' then
    if not plugin.config and not plugin.opts then
      plugin.config = log_plugin_load(name)
    elseif plugin.opts and not plugin.config then
      local opts = plugin.opts
      plugin.config = function()
        log_plugin_load(name)()
        local mod = require((plugin.main or name):gsub('%.nvim$', ''))
        mod.setup(opts)
      end
      plugin.opts = nil
    elseif plugin.config and type(plugin.config) == 'function' then
      local original = plugin.config
      plugin.config = function()
        log_plugin_load(name)()
        original()
      end
...
require('lazy').setup({
  -- NOTE: Plugins can be added with a link (or for a github repo: 'owner/repo' link).
  {
    ...
    event = 'BufReadPre',
    opts = {
      auto_cmd = true, -- enable autocmd
      filetype_exclude = {}, -- fallback to auto for others
      buftype_exclude = {},

      -- You can override shiftwidth/tabstop per filetype using autocmds
    },
    config = function(_, opts)
      require('guess-indent').setup(opts)

      -- Force HTML tab settings
  ...
  --
  {
    ...
    config = function()
      require('colorizer').setup({
        ...
        css_fn = true, -- Enable functions like `rgba(0,0,0,0.5)`
        tailwind = true,
        -- mode = "background", -- Uncomment for background-only highlighting
      })
    end,
  },
  --github copilot
  ...
  }, -- ~/.config/nvim/lua/plugins/fugitive.lua
  {
    ...
    keys = {
      ...
      { '<leader>gd', ':Gdiffsplit<CR>', desc = '[Git] Diff current file' },
      { '<leader>gb', ':Git blame<CR>', desc = '[Git] Blame current file' },
    ...
    init = function()
      vim.cmd [[command! Gedit execute 'edit' fnameescape(expand('<cfile>'))]]
    end,
  },
  {
  ...
  },
  {
    ...
    dependencies = {
      'nvim-lua/plenary.nvim',
      'neovim/nvim-lspconfig', -- this ensures the plugin has lspconfig when it runs
    },
    opts = {
      -- ONLY keep this stuff:
  ...
  { -- Autocompletion
    ...
    dependencies = {
      -- Snippet Engine
      {
        ...
        -- Keep your original build guard for Windows
        build = (function()
        ...
        dependencies = {
          {
            -- community snippets (lazy-loaded so they don't slow startup)
...
--vim.api.nvim_set_keymap("n", "<leader>7", ":Ex<CR>", { noremap = true, silent = true })
